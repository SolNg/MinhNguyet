            ...
            if (/^\**Bí danh[:：]?\**/i.test(n))
              ...
              if (/^\**Từ khóa[:：]?\**/i.test(n))
                s = n
                  ...
                  .map((e) => e.trim())
                  .filter(Boolean);
              else if (!/^\**Ký ức[:：]?\**/i.test(n)) {
                if (
                  ((/^\**Phán đo[aá]n Cốt lõi/i.test(n) ||
                    /^\**Phán định Cốt lõi/i.test(n) ||
                    /^\**Lõi cuối cùng/i.test(n) ||
                    /^\**Cốt lõi Cuối cùng/i.test(n)) &&
                    ((m = !1), (u = !0)),
                  m)
                ) {
                  const e = n.match(/^(\d+)\.\s*(.+)/);
    ...
    h = (function (e) {
      ...
      for (let e = 1; e < u.length; e++) {
        const n = u[e].trim(),
          d = n.split("\n"),
          m = d[0]?.trim() || "";
        if (m.startsWith("Thông tin nền tảng:"))
          t = {
        ...
        else if (m.startsWith("Bảng màu Chữ:")) {
          const e = [];
          let n = "",
            t = "",
            a = "",
          ...
          for (let o = 1; o < d.length; o++) {
            ...
            if (i.startsWith("Màu nền:"))
              ...
              a = i.replace(/^Màu điểm xuyết[:：]\s*/, "").trim();
        ...
        } else if (m.startsWith("Tiếp xúc thân thể:") || m.startsWith("Tiếp xúc Thân thể:"))
          a = U(d.slice(1));
        else if (m.startsWith("Khẩu khí:")) r = U(d.slice(1));
        else if (
          m.startsWith("Biểu đạt cảm xúc:") ||
          m.startsWith("Biểu đạt Cảm xúc:") ||
          n.includes("Vui mừng:") ||
          n.includes("Vui mừng:")
        )
          ...
          m.startsWith("RollSở thích:") ||
          m.startsWith("Không thích")
        )
          for (const e of d) {
            const n = e.trim();
            if (/^\**(?:Không thích|Trong lúc hành sự cực kỳ căm ghét)[:：]?\**/i.test(n)) {
              const e = n.replace(/^\**(?:Không thích|Trong lúc hành sự cực kỳ căm ghét)[:：]?\**\s*/i, "").trim();
              e && "Không có" !== e && l.push(e);
            } else if (/^\**(?:Thích|Giống)[:：]?\**/i.test(n)) {
              const e = n.replace(/^\**(?:Thích|Giống)[:：]?\**\s*/i, "").trim();
                ...
                (function (e) {
                  ...
                  return (
                    ...
                    e.rollDislikes.length > 0 &&
                      n.push(
                        `NSFWKhông thích: ${e.rollDislikes.join("；")}`,
                      ),
                    n.push("</nsfw_dreamtalk>"),
                    n.join("\n")
                  );
